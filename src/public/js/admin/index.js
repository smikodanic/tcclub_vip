...
function formatMoney(money, type) {
    // Convert money to number if it's not already
    ...
    // Add commas for thousands separator to the part before the decimal point
...
function showJoinMember(data) {
    ...
    let bet = data.bet;
    let money = (data.money + data.fee).toLocaleString(); // Use toLocaleString for formatting
    let name = data.bet;
...
function showJoinMember2(data) {
    ...
    let bet = data.join;
    let money = data.money.toLocaleString(); // Use toLocaleString for formatting
    let name = data.bet;
    let time = timerJoin(data.time);
    let result = '';
    result += `
