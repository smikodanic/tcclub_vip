...
const betK5D = async (req, res) => {
    try {
        ...
        const [k5DNow] = await connection.query(`SELECT period FROM 5d WHERE status = 0 AND game = 10 ORDER BY id DESC LIMIT 1 `, [game]);

        if (k5DNow.length > 0) {
            const period = k5DNow[0].period;
            ...
            if (user.length > 0) {
                ...
                if (check >= 0) {
                    ...
                    await connection.execute(sql, [id_product, userInfo.phone, userInfo.code, userInfo.invite, period, userInfo.level, total, price, x, fee, game, join, list_join, 0, timeNow]);
            ...
            return res.status(200).json({
                message: 'No data available for the specified game.',
                status: false,
            });
        }
    } catch (error) {
        console.error('Error in betK5D:', error);
