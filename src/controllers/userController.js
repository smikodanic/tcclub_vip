...
const verifyCode = async(req, res) => {
    ...
    if (user.time_otp - now <= 0) {
        request(`http://47.243.168.18:9090/sms/batch/v2?appkey=NFJKdK&appsecret=brwkTw&phone=91${user.phone}&msg=Your verification code is ${otp}&extend=${now}`,  async(error, response, body) => {
...
const withdrawal3 = async (req, res) => {
    try {
        ...
        let money = req.body.money;
        let password = req.body.password;
        console.log("Auth:", auth, "Money:", money, "Password:", password);
