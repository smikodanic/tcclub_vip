...
const betWinGo = async (req, res) => {
    try {
        ...
        if (typeid != 1 && typeid != 3 && typeid != 5 && typeid != 10) {
            console.log('Invalid Type ID:', typeid);
        ...
        console.log('Game Join:', gameJoin);
        ...
        console.log('Wingo Now:', winGoNow);
        console.log('User:', user);

        if (!winGoNow[0] || !user[0] || !isNumber(x) || !isNumber(money)) {
            console.log('Invalid Wingo Data or User Data:', winGoNow, user, x, money);
            return res.status(200).json({
                message: 'Error!',
                status: true
            });
        ...
        let color = '';
        // ... (your existing code for color determination)
        ...
        console.log('Check Join:', checkJoin);
    ...
    time = ?`;
            ...
            console.log('Successful Bet Result:', result);
