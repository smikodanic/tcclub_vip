        ...
        if (check_u.length == 1 && check_u[0].veri == 1) {
            ...
            if (check_i.length == 1) {
                if (check_ip.length <= 3) {
                    ...
                    return res.status(200).json({
                        message: 'Thanks For Register Drc Lottery',
                        status: true
                    });
                } else {
                    return res.status(200).json({
...
const verifyCodePass = async(req, res) => {
    ...
    if (rows.length == 0) {
        ...
        let user = rows[0];
        if (user.time_otp - now <= 0) {
            request(`http://47.243.168.18:9090/sms/batch/v2?appkey=NFJKdK&appsecret=brwkTw&phone=91${phone}&msg=Your verification code is ${otp}&extend=${now}`,  async(error, response, body) => {
                let data = JSON.parse(body);
                if (data.code == '00000') {
                    await connection.execute("UPDATE users SET otp = ?, time_otp = ? WHERE phone = ? ", [otp, timeEnd, phone]);
                    return res.status(200).json({
